...
| [`components/`](components/) | Exported React design-system components (see below) + their DS-tab preview cards |
...
| [`ui_kits/admin_app/`](ui_kits/admin_app/) | The admin console (gramáž, jedálniček, prevádzky, import…) rebuilt on the brand |

## Components

Reusable React components compiled into `_ds_bundle.js` and exposed on `window.ZdravProjektDesignSystem_970737`. Each has a matching preview card in the **Components** group of the Design System tab. Consume with `const { Button } = window.ZdravProjektDesignSystem_970737`.

- **Button** — pill-shaped brand button; variants `primary` / `secondary` / `ghost` / `danger` / `honey`, sizes `sm` / `md` / `lg`, optional leading icon.
- **Badge** — small status / category pill; tones `green` / `peach` / `teal` / `honey` / `coral` / `orange` / `gray`.
- **Card** — cream surface container with optional header (title, subtitle, actions slot) and padded body.
- **TextField** — labelled form control rendering an input, `multiline` textarea, or `options` select, with the brand focus ring.
- **Toggle** — controlled brand switch (`on` + `onChange`).
- **Checkbox** — labelled controlled checkbox (`on` + `onChange`); pairs with Toggle for boolean form input.
- **Modal** — centered dialog on a blurred scrim, with title, optional icon badge and footer actions.
- **Avatar** — round initials badge with brand gradient tones `green` / `peach` / `teal`, sizes `sm` / `md` / `lg`.
- **SearchBox** — text input with a leading magnifier icon and the brand focus ring.
- **PageHead** — screen header: uppercase eyebrow, display title, description and a right-aligned actions slot.
- **Tabs** — underline tab bar driven by a `tabs` array, controlled via `active` + `onChange`.
- **StatCard** — compact metric surface: a big display number with a label and an optional leading slot (icon/badge).

These primitives are the shared vocabulary behind both `ui_kits/client_app/` and `ui_kits/admin_app/`; the app screens themselves stay as prototypes (they are not exported components).
