...
class TestOrderCRUD:
    ...
    def test_update_order(self, authenticated_client, user):
        ...
        # Initial
        DailyOrder.objects.create(
            user=user, date=today, status='draft',
            data={"lunch": {"menuCounts": {"A": 1}}}
        )
        ...
        response = authenticated_client.post(url, payload, format='json')
        assert response.status_code in [status.HTTP_200_OK, status.HTTP_201_CREATED] # Depending on implementation
    ...
    def test_delete_logic(self, authenticated_client, user):
        ...
        today = date.today()
        DailyOrder.objects.create(
            user=user, date=today, status='submitted',
            data={"lunch": {"menuCounts": {"A": 1}}}
        )
