...
const AdminDashboard: React.FC = () => {
    ...
    const fetchStats = useCallback(async () => {
        setLoading(true);
        try {
            const res = await apiFetch(`${import.meta.env.VITE_API_URL || '/api'}/admin/summary/daily-stats/?date=${date}`);
            if (res.ok) {
    ...
    const renderMealSection = (title: string, icon: string, mealData: MealStats, colorClass: string) => {
        ...
        return (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-full">
                <div className={`p-4 border-b border-gray-100 flex items-center justify-between ${colorClass}`}>
                    ...
                    {hasData && (
                        <span className="text-sm font-semibold bg-white/50 px-2 py-1 rounded-lg text-gray-700">
                             {Object.values(mealData).reduce((acc, cat) => acc + cat.total, 0)} ks
                        </span>
                    )}
                </div>
                ...
                <div className="flex-1 p-0">
                    {!hasData ? (
                        <div className="p-8 text-center text-gray-400 italic text-sm">Žiadne objednávky</div>
                    ) : (
                        <div className="divide-y divide-gray-100">
                            {categories.map(catName => {
                                const catStats = mealData[catName];
                                return (
                                    <div key={catName} className="p-4 hover:bg-gray-50 transition-colors">
                                        <div className="flex justify-between items-center mb-3">
                                            <span className="font-bold text-gray-800 text-sm bg-gray-100 px-2 py-1 rounded text-xs uppercase tracking-wide">{catName}</span>
                                            <span className="text-sm font-bold text-gray-900">{catStats.total} ks</span>
                                        </div>
                                        ...
                                        {Object.keys(catStats.menus).length > 0 && (
                                            <div className="flex flex-wrap gap-2 mb-3">
                                                {Object.entries(catStats.menus).map(([menu, count]) => (
                                                    <div key={menu} className="flex items-center text-xs bg-white border border-gray-200 rounded-md px-2 py-1 shadow-sm">
                                                        <span className="w-5 h-5 flex items-center justify-center bg-gray-100 rounded-full font-bold mr-2 text-[10px] text-gray-600">{menu}</span>
                                        ...
                                        {Object.keys(catStats.diets).length > 0 && (
                                            <div className="flex flex-wrap gap-1.5 mt-2">
                                                {Object.entries(catStats.diets).map(([diet, count]) => (
                                                    <span key={diet} className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-red-50 text-red-700 border border-red-100">
        ...
        <div className="space-y-8 animate-in fade-in duration-500 pb-12">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">Podrobný rozpis objednávok podľa kategórií.</p>
                </div>
                <div className="bg-white p-2 rounded-xl shadow-sm border border-gray-100 flex items-center">
                    <span className="mr-3 text-gray-500 text-sm font-medium pl-2">Dátum:</span>
                    <input 
                        type="date" 
                        value={date}
                ...
                <>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl p-6 text-white shadow-lg shadow-indigo-200">
                            <div className="text-indigo-100 text-sm font-medium mb-1">Spolu Objednávok</div>
                        ...
                        <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm flex flex-col justify-center">
                         ...
                         <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm flex flex-col justify-center">
                             <div className="flex items-center justify-between mb-1">
                                ...
                                <span className="text-2xl font-bold text-gray-600">{stats.status_breakdown?.draft || 0}</span>
                             </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                        {renderMealSection("Raňajky", "☕", stats.meals.breakfast, "bg-orange-50/50")}
                        {renderMealSection("Obed", "🍽️", stats.meals.lunch, "bg-blue-50/50")}
                        {renderMealSection("Olovrant", "🍎", stats.meals.olovrant, "bg-green-50/50")}
                    </div>
                </>
            )}
        </div>
