...
describe('OrderPage Logic & Triggers', () => {
    ...
    it('Copy Olovrant: Copies data from Lunch when triggered', async () => {
        ...
        localStorageMock.setItem(`activeMeals_${date}`, JSON.stringify({ breakfast: false, lunch: true, olovrant: true }));
        ...
        const copySwitch = await within(olovrantCard as HTMLElement).findByRole('switch');
        
        // Verify it's the right one
        expect(within(olovrantCard as HTMLElement).getByText(/Kopírovať z dnešného obeda/i)).toBeInTheDocument();
    ...
    it('Copy Breakfast: Copies from Previous Day Lunch', async () => {
        ...
        localStorageMock.setItem(`order_${yesterdayStr}`, JSON.stringify(prevOrder));
        ...
        localStorageMock.setItem(`activeMeals_${today}`, JSON.stringify({ breakfast: true, lunch: true, olovrant: false }));
        ...
        // 4. Find Copy Switch
        const copySwitch = await within(breakfastCard as HTMLElement).findByRole('switch');
        expect(within(breakfastCard as HTMLElement).getByText(/Auto-výpočet/i)).toBeInTheDocument();
    ...
    it('displays "Termín uplynul" badge when deadline passed', () => {
        (OrderService.checkDeadline as Mock).mockImplementation((_date: string, meal: string) => {
    ...
    it('Submit button is disabled when ALL deadlines passed', () => {
        ...
        expect(screen.getByText('Na tento deň už nie je možné vytvoriť objednávku (termín uplynul).')).toBeVisible();
    });
    ...
    it('Submit button logic (enabled/disabled based on portions)', () => {
        ...
        expect(screen.queryByText('Na tento deň už nie je možné vytvoriť objednávku (termín uplynul).')).not.toBeInTheDocument();
    });
});
